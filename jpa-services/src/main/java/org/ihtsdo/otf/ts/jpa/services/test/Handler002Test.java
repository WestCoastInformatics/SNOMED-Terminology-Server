...
public class Handler002Test {
  ...
  @Test
	public void testHandlerNormalUse001() throws Exception {
	    ...
	    handlerService.resolve(sctConcept, TerminologyUtility.getHierarchicalIsaRels(
	            sctConcept.getTerminology(), sctConcept.getTerminologyVersion()));
	    ...
	    Close the content service
	        TEST: the description should have 2 langauge refset entries, and concept.getDefaultPreferredName should equal "Bipartite ossification"*/
	    ...
	    assertEquals(sctDescription.getConcept().getDefaultPreferredName(), "Bipartite ossification");
	    ...
	    */  
	    ...
	    assertTrue(sctRelationship.getSourceConcept().getDefaultPreferredName().length() > 0);   
	    assertTrue(sctRelationship.getDestinationConcept().getId() > 0); 
	    assertTrue(sctRelationship.getDestinationConcept().getTerminology().length() > 0);  
	    assertTrue(sctRelationship.getDestinationConcept().getTerminologyId().length() > 0); 
	    assertTrue(sctRelationship.getDestinationConcept().getDefaultPreferredName().length() > 0);
	    ...
	    LanguageRefSetMember sctLrm =
	        contentService.getLanguageRefSetMember("0ec8164a-47db-5f44-b3e4-50b33c495859", "SNOMEDCT", "latest");
	    ...
	    /*Open a content service
	    Retrieve concept attributeValue refset member "0123aa2d-f249-54a5-9bed-0a3a4d5b7493" (SNOMEDCT) from the content service.
	    Close the content service
	    ...
	    */
	    ...
	    AttributeValueConceptRefSetMember sctAvrm =
	        (AttributeValueConceptRefSetMember) contentService.getAttributeValueRefSetMember("0123aa2d-f249-54a5-9bed-0a3a4d5b7493", "SNOMEDCT", "latest");
	    ...
	    /*Open a content service
	    Retrieve description attributeValue refset member "00fa0433-ec87-5ddc-a9c1-6fc12055c0e6" (SNOMEDCT) from the content service.
	    Close the content service
	        TEST: the attribute value refset member's description should have id, terminologyId, terminology, term set.	    
	    */
	    contentService = new ContentServiceJpa();
	    AttributeValueDescriptionRefSetMember sctAvdrm =
	        (AttributeValueDescriptionRefSetMember) contentService.getAttributeValueRefSetMember("00fa0433-ec87-5ddc-a9c1-6fc12055c0e6", "SNOMEDCT", "latest");
	    ...
	    /*Open a content service
	    Retrieve concept AssociationReference refset member "00548d88-3544-55ce-9084-7b323ee327e7" (SNOMEDCT) from the content service.
	    Close the content service
	        TEST: the association reference refset member's concept should have id, terminologyId, terminology, defaultPreferredName set.	    
	    */
	    contentService = new ContentServiceJpa();
	    AssociationReferenceConceptRefSetMember sctArcrm =
	        (AssociationReferenceConceptRefSetMember) contentService.getAssociationReferenceRefSetMember("00548d88-3544-55ce-9084-7b323ee327e7", "SNOMEDCT", "latest");
	    ...
	    /*Open a content service
	    Retrieve description AssociationReference refset member "0123aa2d-f249-54a5-9bed-0a3a4d5b7493" (SNOMEDCT) from the content service.
	    Close the content service
	        TEST: the association reference refset member's description should have id, terminologyId, terminology, term set.	    
	    */
	    /** Commented out because currently no data for this condition. */
	    /*contentService = new ContentServiceJpa();
	    AssociationReferenceDescriptionRefSetMember sctArdrm =
	        (AssociationReferenceDescriptionRefSetMember) contentService.getAssociationReferenceRefSetMember("0123aa2d-f249-54a5-9bed-0a3a4d5b7493", "SNOMEDCT", "latest");
	    handlerService.resolve(sctArdrm);
	    contentService.close();
	    ...
	    /*Open a content service
	    Retrieve a complex map refset member "abcac540-87be-5405-9048-f14333f3abc2" (SNOMEDCT) from the content service.
	    Close the content service
	        TEST: the complex map refset member's concept should have id, terminologyId, terminology, defaultPreferredName set.	    
	    */
	    contentService = new ContentServiceJpa();
	    ComplexMapRefSetMember complexMapRefSetMember =
	        contentService.getComplexMapRefSetMember("abcac540-87be-5405-9048-f14333f3abc2", "SNOMEDCT", "latest");
	    ...
	    assertTrue(complexMapRefSetMember.getConcept().getDefaultPreferredName().length() > 0); 	
	    
	    /*Open a content service
	    Retrieve a description type refset member "807f775b-1d66-5069-b58e-a37ace985dcf" (SNOMEDCT) from the content service.
	    Close the content service
	        TEST: the description type refset member's concept should have id, terminologyId, terminology, defaultPreferredName set.	    
	    */
	    contentService = new ContentServiceJpa();
	    DescriptionTypeRefSetMember descriptionTypeRefSetMember =
	        contentService.getDescriptionTypeRefSetMember("807f775b-1d66-5069-b58e-a37ace985dcf", "SNOMEDCT", "latest");
	    handlerService.resolve(descriptionTypeRefSetMember);
	    contentService.close();
	    assertTrue(descriptionTypeRefSetMember.getConcept().getId() > 0); 
	    assertTrue(descriptionTypeRefSetMember.getConcept().getTerminology().length() > 0);  
	    assertTrue(descriptionTypeRefSetMember.getConcept().getTerminologyId().length() > 0); 
	    assertTrue(descriptionTypeRefSetMember.getConcept().getDefaultPreferredName().length() > 0); 	
	    
	    /*Open a content service
	    Retrieve a module dependency refset member "f6431457-161b-5b46-9217-573c20c00070" (SNOMEDCT) from the content service.
	    Close the content service
	        TEST: the module dependency refset member's concept should have id, terminologyId, terminology, defaultPreferredName set.	    
	    */
	    contentService = new ContentServiceJpa();
	    ModuleDependencyRefSetMember moduleDependencyRefSetMember =
	        contentService.getModuleDependencyRefSetMember("f6431457-161b-5b46-9217-573c20c00070", "SNOMEDCT", "latest");
	    handlerService.resolve(moduleDependencyRefSetMember);
	    contentService.close();
	    assertTrue(moduleDependencyRefSetMember.getConcept().getId() > 0); 
	    assertTrue(moduleDependencyRefSetMember.getConcept().getTerminology().length() > 0);  
	    assertTrue(moduleDependencyRefSetMember.getConcept().getTerminologyId().length() > 0); 
	    assertTrue(moduleDependencyRefSetMember.getConcept().getDefaultPreferredName().length() > 0); 
	    
	    /*Open a content service
	    Retrieve a refset descriptor refset member "a6417d6f-5a66-5039-8e94-698d28a4da46" (SNOMEDCT) from the content service.
	    Close the content service
	        TEST: the refset descriptor refset member's concept should have id, terminologyId, terminology, defaultPreferredName set.	    
	    */
	    contentService = new ContentServiceJpa();
	    RefsetDescriptorRefSetMember refsetDescriptorRefSetMember =
	        contentService.getRefsetDescriptorRefSetMember("a6417d6f-5a66-5039-8e94-698d28a4da46", "SNOMEDCT", "latest");
	    handlerService.resolve(refsetDescriptorRefSetMember);
	    contentService.close();
	    assertTrue(refsetDescriptorRefSetMember.getConcept().getId() > 0); 
	    assertTrue(refsetDescriptorRefSetMember.getConcept().getTerminology().length() > 0);  
	    assertTrue(refsetDescriptorRefSetMember.getConcept().getTerminologyId().length() > 0); 
	    assertTrue(refsetDescriptorRefSetMember.getConcept().getDefaultPreferredName().length() > 0); 	 
	    
	    /*Open a content service
	    Retrieve a simple map refset member "0001406e-99ce-5ebe-b84e-99691c094948" (SNOMEDCT) from the content service.
	    Close the content service
	        TEST: the simple map refset member's concept should have id, terminologyId, terminology, defaultPreferredName set.	    
	    */
	    contentService = new ContentServiceJpa();
	    SimpleMapRefSetMember simpleMapRefSetMember =
	        contentService.getSimpleMapRefSetMember("0001406e-99ce-5ebe-b84e-99691c094948", "SNOMEDCT", "latest");
	    ...
	    assertTrue(simpleMapRefSetMember.getConcept().getDefaultPreferredName().length() > 0); 	 	 
	    
	    /*Open a content service
	    Retrieve a simple refset member "06b90ec2-0c9b-5b00-8a64-35a3494cefb4" (SNOMEDCT) from the content service.
	    Close the content service
	        TEST: the simple refset member's concept should have id, terminologyId, terminology, defaultPreferredName set.	    
	    */
	    contentService = new ContentServiceJpa();
	    SimpleRefSetMember simpleRefSetMember =
	        contentService.getSimpleRefSetMember("06b90ec2-0c9b-5b00-8a64-35a3494cefb4", "SNOMEDCT", "latest");
	    ...
	    assertTrue(simpleRefSetMember.getConcept().getDefaultPreferredName().length() > 0); 	 	 	    
   }
  ...
  @Test
	public void testHandlerDegenerateUse001() throws Exception {
		...
		// TEST: exception
		try {
			handlerService.resolve((Concept) null, TerminologyUtility.getHierarchicalIsaRels(
		            "SNOMEDCT", "latest"));
  ...
  public void testHandlerEdgeCases001() throws Exception {
	  ...
	  handlerService.resolve(new AssociationReferenceDescriptionRefSetMemberJpa());
